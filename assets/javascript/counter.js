...
class Counter {
  ...
  calculateTotalDays(start_date, end_date) {
    return (
      this.convertToTimeStamp(end_date) -
      this.convertToTimeStamp(start_date)
    ) / (1000 * 3600 * 24);
  }

  calculatePastDays(current_date, start_date) {
    return (
      this.convertToTimeStamp(current_date) -
      this.convertToTimeStamp(start_date)
    ) / (1000 * 3600 * 24);
  }

  calculateMissingDays(total_days, current_date, start_date) {
    return total_days - this.calculatePastDays(current_date, start_date);
  }

  // Time

  // To calculate the time difference of two dates
  calculateTotalTime(end_date, start_date) {
    return (
      this.convertToTimeStamp(end_date) -
      this.convertToTimeStamp(start_date)
    )
  ...
  calculatePastTime(current_time, start_date) {
    return (
      this.convertToTimeStamp(current_time) -
      this.convertToTimeStamp(start_date)
    )
  }

  calculateMissingTime(total_time, current_time, start_date) {
    return total_time - this.calculatePastTime(current_time, start_date);
  }
  ...
  calculatePercentageInTime(total_time, current_time, start_date) {
    return {
      "missing_days_percent": this.percentage(
        this.calculateMissingTime(total_time, current_time, start_date), total_time
      ),
      "past_days_percent": this.percentage(
        this.calculatePastTime(current_time, start_date), total_time
      )
    }
  }

  countdown() {
    this.config.distance = this.calculateMissingTime(this.config.total_time, this.getCurrentDate().date_time, this.config.start_date);
  ...
  progressCounterBar() {
    this.config.distance = this.calculateMissingTime(this.config.total_time, this.getCurrentDate().date_time, this.config.start_date);
    const missing_perc = this.calculatePercentageInTime(this.config.total_time, this.getCurrentDate().date_time, this.config.start_date).missing_days_percent;
    const past_perc = this.calculatePercentageInTime(this.config.total_time, this.getCurrentDate().date_time, this.config.start_date).past_days_percent;
  ...
  start() {
    ...
    this.config.total_days = this.calculateTotalDays(this.config.start_date, this.config.end_date);
    this.config.total_time = this.calculateTotalTime(this.config.end_date, this.config.start_date);
